// frontend-next/src/components/auth/LoginClient.jsx
'use client';
...
function GoogleLoginButton({ disabled, onAccessToken }) {
  ...
  return (
    <div
      ...
      onKeyDown={(e) => {
        if ((e.key === 'Enter' || e.key === ' ') && !disabled) googleLogin();
      }}
      className={`flex items-center justify-center bg-customPurple rounded-lg shadow-md cursor-pointer transition-all duration-200 ease-in-out w-full hover:shadow-lg mb-3 ${
        disabled ? 'opacity-60 cursor-not-allowed' : ''
      }`}
    >
      <div className="py-2 px-4 flex items-center justify-center">
        <img className="w-8 h-8 above-1000:w-6 above-1000:h-6" src="/images/google.png" alt="Google" />
      </div>
...
export default function LoginClient() {
  ...
  return (
    <div className="container mx-auto px-2 flex-colo min-h-auto md:min-h-[calc(100vh-200px)] py-3">
      ...
      >
        <img src="/images/MOVIEFROST.png" alt="logo" className="w-full h-10 object-contain mb-1" />
        ...
        {googleEnabled ? (
          <>
            <GoogleLoginButton disabled={loading} onAccessToken={submitGoogle} />
        ...
        <div className="w-full flex flex-col gap-1">
          ...
          />
          {fieldErrors.password ? <InlineError text={fieldErrors.password} /> : null}
        </div>

        <button
          type="submit"
          disabled={!canSubmit}
          className="bg-customPurple transitions hover:bg-main flex-rows mt-6 mb-4 gap-4 text-white font-semibold p-4 rounded-lg w-full disabled:opacity-60"
        >
          {loading ? 'Loading...' : (<><FiLogIn /> Sign In</>)}
        </button>

        <p className="text-center text-border text-sm">
          Don&apos;t have an account?{' '}
          <Link href="/register" className="text-customPurple font-semibold ml-2 hover:underline">
            Sign Up
          </Link>
        </p>
      </form>
